...
int main(int argc, char** argv)
{
    ...
    node->declare_parameter<uint16_t>("frequency", setting.frequency);
    ...
    node->get_parameter("frequency", setting.frequency);

    ldlidar::LDLidarDriver* ldlidarnode = new ldlidar::LDLidarDriver();

    RCLCPP_INFO(node->get_logger(), "LDLiDAR SDK Pack Version is: %s",
    ...
    RCLCPP_INFO(node->get_logger(), "<laser_scan_dir>: %s",
                (setting.laser_scan_dir ? "Counterclockwise" : "Clockwise"));
    RCLCPP_INFO(node->get_logger(), "<enable_angle_crop_func>: %s",
    ...
    RCLCPP_INFO(node->get_logger(), "<frequency>: %u", setting.frequency);
    ...
    rclcpp::WallRate r(setting.frequency);  // 10hz
                               ...
                               rclcpp::Publisher<sensor_msgs::msg::LaserScan>::SharedPtr& lidarpub)
{
    ...
    if (lidar_spin_freq > 0) {
        ...
        for (auto point : src) {
            ...
            if (setting.enable_angle_crop_func) {  // Angle crop setting, Mask data within the set
                                                   // angle range
